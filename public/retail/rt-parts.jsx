/* Koomzo Retail POS — shared board parts */
const RT_NAV = [
...
function Rail({ view, onView, held }) {
  return (
    <nav className="rail">
      ...
      {RT_NAV.map((n) => (
        <button key={n.id} className={view === n.id ? 'on' : ''} onClick={() => onView(n.id)} title={n.label}>
          ...
          {n.id === 'tickets' && held > 0 && <i className="dot">{held}</i>}
        </button>
      ))}
...
function Cart({ cls, orderNo, items, customer, totals, profile, selUid, onSelect, onQty, onClear, onHold,
  ...
  return (
    <aside className={cls}>
      <div className="cart__head">
        ...
        <button className="icbtn" title="Hold ticket" onClick={onHold}><ion-icon name="pause-outline"></ion-icon></button>
      ...
      <div className="totals">
        ...
        <div className="trow"><span className="k">Tax ({Math.round(profile.tax * 100)}%)</span><span className="v">{money(totals.tax)}</span></div>
...
Object.assign(window, { RT_NAV, Rail, TopBar, EntryBar, Cats, Tile, CatalogGrid, CartLine, Cart, DockBar });
